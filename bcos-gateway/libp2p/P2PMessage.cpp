  ...
  do {                                                                         \
    if ((offset) > (length)) {                                                 \
      throw std::out_of_range(                                                 \
          "Out of range error, offset:" + std::to_string(offset) +             \
...
bool P2PMessageOptions::encode(bytes &_buffer) {
  ...
  if (!m_srcNodeID || m_srcNodeID->empty() ||
      (m_srcNodeID->size() > MAX_NODEID_LENGTH)) {
    P2PMSG_LOG(ERROR) << LOG_DESC("srcNodeID length valid")
                      << LOG_KV("srcNodeID length",
                                (m_srcNodeID ? m_srcNodeID->size() : 0));
    return false;
  }
  ...
  for (auto dstNodeID : m_dstNodeIDs) {
    if (!dstNodeID || dstNodeID->empty() ||
        (dstNodeID->size() > MAX_NODEID_LENGTH)) {
      P2PMSG_LOG(ERROR) << LOG_DESC("dstNodeID length valid")
                        << LOG_KV("dstNodeID length",
                                  (dstNodeID ? dstNodeID->size() : 0));
      return false;
    }
  }
  ...
  uint16_t groupIDLength =
      boost::asio::detail::socket_ops::host_to_network_short(
          (uint16_t)m_groupID.size());
  _buffer.insert(_buffer.end(), (byte *)&groupIDLength,
                 (byte *)&groupIDLength + 2);
  // groupID
  ...
  uint16_t nodeIDLength =
      boost::asio::detail::socket_ops::host_to_network_short(
          (uint16_t)m_srcNodeID->size());
  _buffer.insert(_buffer.end(), (byte *)&nodeIDLength,
                 (byte *)&nodeIDLength + 2);
  // srcNodeID
  ...
  _buffer.insert(_buffer.end(), (byte *)&dstNodeIDCount,
                 (byte *)&dstNodeIDCount + 1);
...
ssize_t P2PMessageOptions::decode(bytesConstRef _buffer) {
  ...
  try {
    ...
    m_srcNodeID->insert(m_srcNodeID->begin(), (byte *)&_buffer[offset],
                        (byte *)&_buffer[offset] + nodeIDLength);
    offset += nodeIDLength;
    ...
    CHECK_OFFSET_WITH_THROW_EXCEPTION(offset + dstNodeCount * nodeIDLength,
                                      length);
    // dstNodeIDs
...
bool P2PMessage::encode(bytes &_buffer) {
  ...
  uint32_t length = 0;
  uint16_t version =
      boost::asio::detail::socket_ops::host_to_network_short(m_version);
  uint16_t packetType =
      boost::asio::detail::socket_ops::host_to_network_short(m_packetType);
...
ssize_t P2PMessage::decodeHeader(bytesConstRef _buffer) {
  ...
  // length field
  m_length = boost::asio::detail::socket_ops::network_to_host_long(
      *((uint32_t *)&_buffer[offset]));
  offset += 4;
  ...
  // packetType
  m_packetType = boost::asio::detail::socket_ops::network_to_host_short(
      *((uint16_t *)&_buffer[offset]));
  offset += 2;

  // seq
  m_seq = boost::asio::detail::socket_ops::network_to_host_long(
      *((uint32_t *)&_buffer[offset]));
  offset += 4;
