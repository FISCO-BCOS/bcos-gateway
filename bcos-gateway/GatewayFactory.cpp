...
void GatewayFactory::initCert2PubHexHandler() {
  ...
  auto handler = [](const std::string &_cert, std::string &_pubHex) -> bool {
    ...
    do {
      ...
      auto hex =
          bcos::toHexString(pubKey->data, pubKey->data + pubKey->length, "");
      _pubHex = *hex.get();
  ...
  auto handler = [](X509 *x509, std::string &_pubHex) -> bool {
    ...
    if (pubKey == NULL) {
      GATEWAY_FACTORY_LOG(ERROR) << LOG_DESC(
          "initSSLContextPubHexHandler X509_get0_pubkey_bitstr error");
      return false;
    }
 ...
 */
  ...
  if (!caCertContent || caCertContent->empty()) {
    ...
    BOOST_THROW_EXCEPTION(InvalidParameter() << errinfo_comment(
                              "buildSSLContext: unable read content of ca: " +
                              _certConfig.caCert));
  }
...
std::shared_ptr<boost::asio::ssl::context> GatewayFactory::buildSSLContext(
  ...
  if (SSL_CTX_use_enc_PrivateKey_file(sslContext->native_handle(),
                                      ...
                                      SSL_FILETYPE_PEM) <= 0) {
    GATEWAY_FACTORY_LOG(ERROR)
        << LOG_DESC("SSL_CTX_use_enc_PrivateKey_file error");
    BOOST_THROW_EXCEPTION(InvalidParameter() << errinfo_comment(
                              "GatewayFactory::buildSSLContext "
...
GatewayFactory::buildGateway(const std::string &_configPath) {
 ...
 */
std::shared_ptr<Gateway>
GatewayFactory::buildGateway(GatewayConfig::Ptr _config) {
  try {

    std::string nodeCert = (_config->smSSL() ? _config->smCertConfig().nodeCert
    ...
    host->setThreadPool(
        std::make_shared<ThreadPool>("P2P", _config->threadPoolSize()));
    ...
    GATEWAY_FACTORY_LOG(INFO)
        << LOG_DESC("GatewayFactory::init") << LOG_KV("myself pub id", pubHex);
    ...
    service->registerDisconnectHandler(
        [weakptrGatewayNodeManager](NetworkException e,
