...
public:
  ...
  bool parseReceivedJson(
      const std::string &_json, uint32_t &statusSeq,
      std::unordered_map<std::string, std::set<std::string>> &nodeIDsMap);
  void
  updateNodeIDs(const P2pID &_p2pID, uint32_t _seq,
                const std::unordered_map<std::string, std::set<std::string>>
                    &_nodeIDsMap);

  void onReceiveStatusSeq(const P2pID &_p2pID, uint32_t _statusSeq,
  ...
  bool queryP2pIDs(const std::string &_groupID, const std::string &_nodeID,
  ...
  queryFrontServiceInterfaceByGroupID(const std::string &_groupID);
  /**
...
private:
  ...
  std::unordered_map<std::string,
                     std::unordered_map<std::string, std::set<P2pID>>>
      m_peerGatewayNodes;
  // P2pID => statusSeq
