...
public:
  ...
  virtual void onConnect(NetworkException e, P2PInfo const &p2pInfo,
  ...
  sendMessageByNodeID(P2pID nodeID,
                      std::shared_ptr<P2PMessage> message) override;
  void sendMessageBySession(int _packetType, bytesConstRef _payload,
  ...
  void asyncBroadcastMessage(std::shared_ptr<P2PMessage> message,
  ...
  virtual void
  setMessageFactory(std::shared_ptr<MessageFactory> _messageFactory) {
  ...
  void updateStaticNodes(std::shared_ptr<SocketFace> const &_s,
                         P2pID const &nodeId);
  ...
  std::shared_ptr<P2PSession>
  getP2PSessionByNodeId(P2pID const &_nodeID) override {
...
private:
  std::vector<std::function<void(NetworkException, P2PSession::Ptr)>>
      m_disconnectionHandlers;
