...
public:
  ...
  virtual void
      asyncSendMessage(Message::Ptr, Options = Options(),
  ...
  virtual void setSocket(std::shared_ptr<SocketFace> socket) {
    m_socket = socket;
  }
  ...
  virtual std::function<void(NetworkException, SessionFace::Ptr, Message::Ptr)>
  messageHandler() {
    return m_messageHandler;
  }
  virtual void setMessageHandler(
      std::function<void(NetworkException, SessionFace::Ptr, Message::Ptr)>
...
private:
  ...
  std::shared_ptr<std::unordered_map<uint32_t, ResponseCallback::Ptr>>
      m_seq2Callback;
...
public:
  ...
  virtual std::shared_ptr<SessionFace>
  create_session(std::weak_ptr<Host> _server,
