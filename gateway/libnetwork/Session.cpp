...
#include "SessionFace.h"   // for Respon...
...
Session::Session(size_t _bufferSize) : bufferSize(_bufferSize) {
  ...
  m_seq2Callback =
      std::make_shared<std::unordered_map<uint32_t, ResponseCallback::Ptr>>();
}
...
bool Session::actived() const {
  ...
  return m_actived && server && server->haveNetwork() && m_socket &&
         m_socket->isConnected();
}

void Session::asyncSendMessage(Message::Ptr message, Options options,
...
void Session::send(std::shared_ptr<bytes> _msg) {
  ...
  SESSION_LOG(TRACE) << "send"
                     << LOG_KV("writeQueue size", m_writeQueue.size());
  {
    ...
    if (server && server->haveNetwork()) {
      if (m_socket->isConnected()) {
        ...
        server->asioInterface()->asyncWrite(
            ...
            boost::bind(&Session::onWrite, session,
...
void Session::drop(DisconnectReason _reason) {
  ...
  for (auto &it : *m_seq2Callback) {
    ...
    if (it.second->callback) {
      SESSION_LOG(TRACE) << "drop, call callback by seq"
                         << LOG_KV("seq", it.first);
      if (server) {
        auto callback = it.second;
        server->threadPool()->enqueue([callback, errorCode, errorMsg]() {
          callback->callback(NetworkException(errorCode, errorMsg),
                             Message::Ptr());
        });
      }
    }
  ...
  if (m_socket->isConnected()) {
    try {
      if (_reason == DisconnectRequested || _reason == DuplicatePeer ||
        ...
        SESSION_LOG(DEBUG) << "[drop] closing remote "
      ...
      shutdown_timer->async_wait([socket](
        ...
        if (error && error != boost::asio::error::operation_aborted) {
          SESSION_LOG(WARNING) << "[drop] shutdown timer error"
                               << LOG_KV("errorValue", error.value())
                               << LOG_KV("message", error.message());
        }
      ...
      socket->sslref().async_shutdown(
          [socket, shutdown_timer](const boost::system::error_code &error) {
            ...
            if (error) {
              SESSION_LOG(WARNING) << "[drop] shutdown failed "
                                   << LOG_KV("errorValue", error.value())
                                   << LOG_KV("message", error.message());
            }
...
void Session::doRead() {
  ...
  if (m_actived && server && server->haveNetwork()) {
    ...
    auto asyncRead = [self](boost::system::error_code ec,
                            std::size_t bytesTransferred) {
      auto s = self.lock();
      if (s) {
        if (ec) {
          SESSION_LOG(WARNING) << LOG_DESC("doRead error")
                               << LOG_KV("endpoint", s->nodeIPEndpoint())
                               << LOG_KV("message", ec.message());
          s->drop(TCPError);
          return;
        }
        ...
        while (true) {
          ...
          ssize_t result = message->decode(
              bytesConstRef(s->m_data.data(), s->m_data.size()));
          if (result > 0) {
            ...
            s->onMessage(NetworkException(P2PExceptionType::ProtocolError,
                                          "ProtocolError"),
                         message);
            break;
          }
    ...
    if (m_socket->isConnected()) {
      server->asioInterface()->asyncReadSome(
          m_socket, boost::asio::buffer(m_recvBuffer, m_recvBuffer.size()),
          asyncRead);
    } else {
    ...
    SESSION_LOG(ERROR) << LOG_DESC(
                              "callback doRead failed for session inactived")
...
bool Session::checkRead(boost::system::error_code _ec) {
  if (_ec && _ec.category() != boost::asio::error::get_misc_category() &&
    ...
    SESSION_LOG(WARNING) << LOG_DESC("checkRead error")
...
void Session::onMessage(NetworkException const &e, Message::Ptr message) {
  ...
  if (m_actived && server && server->haveNetwork()) {
    ...
    if (callbackPtr && message->isRespPacket()) {
      ...
      if (m_messageHandler) {
        SESSION_LOG(TRACE)
            << "onMessage can't find callback, call default messageHandler"
        ...
        SESSION_LOG(WARNING)
            << "onMessage can't find callback and default messageHandler";
      }
    }
  }
}
...
void Session::updateIdleTimer(
  ...
  if (_idleTimer) {
    _idleTimer->expires_from_now(
        boost::posix_time::seconds(m_idleTimeInterval));
...
void Session::setHost(std::weak_ptr<Host> host) {
  ...
  if (server && server->haveNetwork()) {
    ...
    SESSION_LOG(ERROR) << LOG_DESC(
        "create idleTimer failed for the host has no network");
  }
}
