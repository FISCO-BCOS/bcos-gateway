...
class ASIOInterface {
public:
  enum ASIO_TYPE { TCP_ONLY = 0, SSL = 1 };
  ...
  virtual std::shared_ptr<ba::ssl::context> sslContext() {
    return m_sslContext;
  }
  ...
  virtual std::shared_ptr<boost::asio::deadline_timer>
  newTimer(uint32_t timeout) {
    return std::make_shared<boost::asio::deadline_timer>(
        *m_ioService, boost::posix_time::milliseconds(timeout));
  }

  virtual std::shared_ptr<SocketFace>
  newSocket(NodeIPEndpoint nodeIPEndpoint = NodeIPEndpoint()) {
  ...
  virtual void
  asyncAccept(std::shared_ptr<SocketFace> socket, Handler_Type handler,
              boost::system::error_code = boost::system::error_code()) {
    m_acceptor->async_accept(socket->ref(), m_strand->wrap(handler));
  }

  virtual void asyncResolveConnect(std::shared_ptr<SocketFace> socket,
  ...
  virtual void
  asyncWait(boost::asio::deadline_timer *m_timer,
  ...
  virtual void setVerifyCallback(std::shared_ptr<SocketFace> socket,
                                 VerifyCallback callback, bool = true) {
