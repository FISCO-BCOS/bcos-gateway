...
void Host::startAccept(boost::system::error_code boost_error) {
  ...
  if (m_run) {
    HOST_LOG(INFO) << LOG_DESC("P2P StartAccept")
                   << LOG_KV("Host", m_listenHost) << ":" << m_listenPort;
    ...
    m_asioInterface->asyncAccept(
        ...
        [=](boost::system::error_code ec) {
          ...
          m_asioInterface->setVerifyCallback(
              socket, newVerifyCallback(endpointPublicKey));
          m_asioInterface->asyncHandshake(
              socket, ba::ssl::stream_base::server,
              boost::bind(&Host::handshakeServer, shared_from_this(),
                          ba::placeholders::error, endpointPublicKey, socket));
...
Host::newVerifyCallback(std::shared_ptr<std::string> nodeIDOut) {
  auto host = std::weak_ptr<Host>(shared_from_this());
  return [host, nodeIDOut](bool preverified,
    ...
    try {
      ...
      int crit = 0;
      BASIC_CONSTRAINTS *basic = (BASIC_CONSTRAINTS *)X509_get_ext_d2i(
          cert, NID_basic_constraints, &crit, NULL);
      if (!basic) {
      ...
      HOST_LOG(ERROR) << LOG_DESC("Cert verify failed")
...
std::string Host::obtainCommonNameFromSubject(std::string const &subject) {
  ...
  boost::split(fields, subject, boost::is_any_of("/"),
               boost::token_compress_on);
  for (auto field : fields) {
    std::size_t pos = field.find("CN");
    if (pos != std::string::npos) {
      std::vector<std::string> cn_fields;
      boost::split(cn_fields, field, boost::is_any_of("="),
                   boost::token_compress_on);
...
void Host::obtainNodeInfo(NodeInfo &info, std::string const &node_info) {
  ...
  boost::split(node_info_vec, node_info, boost::is_any_of("#"),
               boost::token_compress_on);
  ...
  HOST_LOG(INFO) << "obtainNodeInfo " << LOG_KV("node_info", node_info)
...
void Host::startPeerSession(
    NodeInfo const &nodeInfo, std::shared_ptr<SocketFace> const &socket,
  ...
  m_threadPool->enqueue([ps, connectionHandler, nodeInfo]() {
    if (connectionHandler) {
      ...
      HOST_LOG(WARNING) << LOG_DESC(
          "No connectionHandler, new connection may lost");
    }
  });
  HOST_LOG(INFO) << LOG_DESC("startPeerSession, Remote=")
...
void Host::asyncConnect(NodeIPEndpoint const &_nodeIPEndpoint,
  ...
  HOST_LOG(INFO) << LOG_DESC("Connecting to node")
  ...
  std::shared_ptr<SocketFace> socket =
      m_asioInterface->newSocket(_nodeIPEndpoint);
  ...
  connect_timer->async_wait([=](const boost::system::error_code &error) {
    ...
    if (error == boost::asio::error::operation_aborted) {
      HOST_LOG(DEBUG) << LOG_DESC(
          "AsyncConnect handshake handler revoke this operation");
      return;
    }
  ...
  m_asioInterface->asyncResolveConnect(
        ...
        if (ec) {
          ...
          m_threadPool->enqueue([callback, _nodeIPEndpoint]() {
            callback(NetworkException(ConnectError, "Connect failed"),
                     NodeInfo(), std::shared_ptr<SessionFace>());
          });
          return;
        } else {
          ...
          m_asioInterface->asyncHandshake(
              ...
              boost::bind(&Host::handshakeClient, shared_from_this(),
                          ba::placeholders::error, socket, endpointPublicKey,
